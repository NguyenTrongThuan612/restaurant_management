...
from app_core.models.user import User, UserRole, UserGender
...
class UpdateUserSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ['fullname', 'birth_date', 'gender', 'email', 'phone', 'status']
        extra_kwargs = {
    ...
    def validate(self, data):
        ...
        employee_allowed_fields = {'fullname', 'birth_date', 'gender', 'phone'}
        manager_only_fields = {'email', 'status'}

        attempted_fields = set(data.keys())

        if request.user.role == UserRole.MANAGER:
            invalid_fields = attempted_fields - manager_only_fields
            if invalid_fields:
                raise serializers.ValidationError(
                    f"Manager only allowed to update fields: {', '.join(manager_only_fields)}"
                )
        else:
            if request.user.id != self.instance.id:
                raise serializers.ValidationError("You can only update your own information")

            invalid_fields = attempted_fields - employee_allowed_fields
            if invalid_fields:
                raise serializers.ValidationError(
                    f"You are only allowed to update fields: {', '.join(employee_allowed_fields)}"
                )
